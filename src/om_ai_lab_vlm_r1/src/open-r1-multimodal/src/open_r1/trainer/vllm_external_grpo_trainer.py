...
class VLLMExternalGRPOTrainer(Trainer):
    """
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask,
        ...
        #print(f"({rank}) In _get_per_token_logps: input_ids: {input_ids.shape}")
        #print(f"({rank}) In _get_per_token_logps: pixel_values: {pixel_values.shape}")
        #print(f"({rank}) In _get_per_token_logps: image_grid_thw: {image_grid_thw}")
        ...
        if rank == 0:
            print(f"logits after truncation: {logits.shape}")
        ...
        #print(f"Shape of logits for generated tokens: {logits.shape}")
    ...
    def _generate_and_score_completions(
        ...
        #print(f"({rank}) Prompts text: {prompts_text}")
        ...
        for x in inputs:
            if "image_path" in x and x["image_path"] is not None:
                ...
                img_paths = [p for p in self._get_key_from_inputs(x, "image_path")]
                assert len(img_paths) == 1, f"Example {x} contains more than one image which is not supported atm"
        ...
        #print(f"Image paths: {image_paths}, {len(image_paths)}")
        ...
        #print(f"({rank}) prompt inputs: {prompt_inputs}")
        #print(f"prompts_text after processing_class: {prompts_text}")

        prompt_inputs = super()._prepare_inputs(prompt_inputs)

        prompt_ids, prompt_mask = prompt_inputs["input_ids"], prompt_inputs["attention_mask"]

        #print(f"({rank}) Prompt ids: {prompt_ids.shape}")
        #print(f"({rank}) image token count: {(prompt_ids == 151655).sum(dim=1)}")
        ...
        if self.use_vllm:
            ...
            #print(f"({rank}) image_paths: {image_paths}")
            all_prompts_text = gather_object(prompts_text)
            all_image_paths = gather_object(image_paths)
            #print(f"({rank}) all_image_paths: {all_image_paths}")
            ...
            if self.accelerator.is_main_process:
                ...
                #print(f"({rank}) ordered_set_of_image_paths: {ordered_set_of_image_paths}")
            ...
            truncated = 0
            for idx, comp in enumerate(completion_ids):
                if len(comp) == 256:
                    truncated = idx
                    print(f"Found truncated generation: {truncated}")

            # Pad the completions, and concatenate them with the prompts
            completion_ids = [torch.tensor(ids, device=device) for ids in completion_ids]
            if rank == 0:
                print(f"completion ID shape: {completion_ids[truncated].shape}")
                print(f"completion ID: {completion_ids[truncated]}")
            completion_ids = pad(completion_ids, padding_value=self.processing_class.pad_token_id)
            #if rank == 0:
            #    print(f"After pad: First completion ID shape: {completion_ids[0].shape}")
            #    print(f"After pad: First completion ID: {completion_ids[0]}")
            #print(f"({rank}) Completion ids after padding: {completion_ids.shape})")
            prompt_completion_ids = torch.cat([prompt_ids, completion_ids], dim=1)

            #print(f"({rank}) Prompt Completion ids: {prompt_completion_ids.shape})")
        else:
        ...
        if rank == 0:
            #print(f"Completion mask full: {completion_mask}")
            print(f"Completion mask row: {completion_mask[truncated]}")
        ...
        if rank == 0:
            print(f"shape of prompt mask: {prompt_mask.shape}")
        ...
        #print(f"logits to keep: {logits_to_keep}")
    ...
    def _compute_loss(self, model, inputs):
        ...
        per_token_logps = self._get_per_token_logps(model, input_ids, attention_mask, logits_to_keep, pixel_values,
                                                    image_grid_thw)
