...
class VLMGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            #print(f"prompt_inputs: {prompt_inputs}")
            #print(self.processing_class.batch_decode(prompt_inputs["input_ids"], skip_special_tokens=False))
            print(f"shape of input ids in grpo_trainer: {prompt_inputs["input_ids"].shape}")

            # Count the occurrences of 151655 in prompt_inputs["input_ids"]
            count_151655 = (prompt_inputs["input_ids"] == 151655).sum(dim=1)
            print(f"Occurrences of 151655 in prompt_inputs['input_ids']: {count_151655}")

            print(f"begin img: {torch.where(prompt_inputs['input_ids'] == 151652)}")
            print(f"end img: {torch.where(prompt_inputs['input_ids'] == 151653)}")
