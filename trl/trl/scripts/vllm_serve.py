...
from trl.extras.custom_process_group import CustomStatelessProcessGroup
...
class ScriptArguments:
    ...
    enforce_eager: Optional[bool] = field(
        default=False,
        metadata={
            "help": "If True, no CUDA graph will be generated and the inference is performed eagerly. "
        },
    )
    limit_image_per_prompt: Optional[int] = field(
        default=1,
        metadata={
            "help": "The number of images allowed per prompt. "
        },
    )
    limit_video_per_prompt: Optional[int] = field(
        default=0,
        metadata={
            "help": "The number of videos allowed per prompt. "
        }
    )
    max_pixels: Optional[int] = field(
        default = 12845056,
        metadata={
            "help": "The max number of pixels used. Should be a multiple of 28*28"
        }
    )
    min_pixels: Optional[int] = field(
        default = 3136,
        metadata={
            "help": "The min number of pixels used. Should be a multiple of 28*28"
        }
    )
...
def main(script_args: ScriptArguments):
    ...
    llm = LLM(
        ...
        disable_custom_all_reduce=script_args.disable_custom_all_reduce,
        enforce_eager=script_args.enforce_eager,
        limit_mm_per_prompt={"image": script_args.limit_image_per_prompt,
                             "video": script_args.limit_video_per_prompt},
        mm_processor_kwargs={"max_pixels": script_args.max_pixels,
    ...
    class GenerateMultimodalRequest(BaseModel):
        prompts: list[dict[str, Union[str, list[str]]]]
        n: int = 1
        repetition_penalty: float = 1.0
        temperature: float = 1.0
        top_p: float = 1.0
    ...
    class TextIdResponse(BaseModel):
        completion_ids: list[list[int]]

    @app.post("/generate_from_multimodal_input/", response_model=TextIdResponse)
    async def generate(request: GenerateMultimodalRequest):
        """
        Generates completions for the provided prompts.
        ...
        all_outputs = llm.generate(inputs_with_image, sampling_params=sampling_params,
                                   #tools=tools
                                   )
        completion_ids = [list(output.token_ids) for outputs in all_outputs for output in outputs.outputs]
        return {"completion_ids": completion_ids}

    class GenerateMultimodalTokenRequest(BaseModel):
        prompts: list[dict[str, Union[list[int], str, list[str]]]]
        n: int = 1
        repetition_penalty: float = 1.0
        temperature: float = 1.0
        top_p: float = 1.0
    ...
    @app.post("/generate_from_multimodal_token_input/", response_model=TextIdResponse)
    async def generate(request: GenerateMultimodalTokenRequest):
        ...
        for entry in request.prompts:
            ...
            for img_path in entry["image_path"]:
                ...
                try:
                    ...
                    logger.info(f"Warning: could not process image {img_path}: {e}")
                images.append(img)
            inputs_with_image.append(TokensPrompt(prompt_token_ids=entry["prompt_token_ids"],
                                                  multi_modal_data={"image": images}))
            #inputs_with_image.append(
            #    {"prompt": entry["prompt"],
            #     "multi_modal_data": {"image": images}
