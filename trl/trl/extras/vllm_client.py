...
class VLLMClient:
    ...
    def generate_from_multimodal_input(
        self,
        prompts: list[dict[str, Union[list[str], str]]],
        n: int = 1,
        repetition_penalty: float = 1.0,
        temperature: float = 1.0,
        top_p: float = 1.0,
        ...
        Args:
            prompts (`list[str]`):
                List of text prompts with image paths for which the model will generate completions.
            n (`int`, *optional*, defaults to `1`):
                Number of completions to generate for each prompt.
            repetition_penalty (`float`, *optional*, defaults to `1.0`):
                Parameter for repetition penalty. 1.0 means no penalty.
        ...
        url = f"http://{self.host}:{self.server_port}/generate_from_multimodal_input/"
        response = self.session.post(
            url,
            json={
                "prompts": prompts,
        ...
        if response.status_code == 200:
            ...
            return as_json["completion_ids"]
        else:
            raise Exception(f"Request failed: {response.status_code}, {response.text}")

    def generate_from_multimodal_token_input(
            self,
            prompts: list[dict[str, Union[list[int], list[str], str]]],
            n: int = 1,
            repetition_penalty: float = 1.0,
            temperature: float = 1.0,
            top_p: float = 1.0,
        ...
        url = f"http://{self.host}:{self.server_port}/generate_from_multimodal_token_input/"
        response = self.session.post(
            url,
            json={
                "prompts": prompts,
    ...
    def init_communicator(self):
        ...
        # close weight update group, otherwise we can have to restart the server everytime the training fails
